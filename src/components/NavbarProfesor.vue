  ...
  >
    ...
    <div class="offcanvas-body">
      ...
      <div class="text-center mb-4">
        ...
        <h5 class="mt-2">{{ professorName }}</h5>
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <router-link class="nav-link" to="/coursesp">
...
export default {
  ...
  data() {
    return {
      ...
      professorName: '', // Professor's name to display
      offcanvasOpen: false,
  ...
  methods: {
    ...
    async fetchProfessorName() {
      if (!this.userId) {
        console.error('User ID is not set. Cannot fetch professor name.')
        this.professorName = 'Error Loading Name'
        return
      }
      ...
      try {
        ...
        this.professorName = response.data.full_name || 'Unknown Professor'
      } catch (error) {
        console.error('Error fetching professor name:', error.response?.data || error.message)
        this.professorName = 'Error Loading Name'
      }
    },
    toggleSearch() {
  ...
  mounted() {
    ...
    if (this.userId) {
      this.fetchProfessorName() // Fetch name only after userId is set
    } else {
